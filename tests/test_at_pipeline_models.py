...
class TestCasesModels:
    ...
    def test_cases_doc_validation(self):
        ...
        doc = CasesDoc(cases=[CaseSuite(id="s1", name="S", module="M")])
        data = doc.model_dump()
...
class TestMappingModels:
    ...
    def test_unmapped_entry(self):
        ...
        entry = MappingEntry(
            ...
            fix_suggestion="add setAccessibleName",
        )
    ...
    def test_mappings_doc(self):
        ...
        doc = ElementMappingsDoc(
            mappings=[
                MappingEntry(
                    case_id="s1", step_index=0, description="test", step_type=StepType.action
                )
            ],
        )
