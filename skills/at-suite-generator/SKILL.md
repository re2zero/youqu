...
4. **Generate** — one sub-agent per slice maps cases → suites, picking `selector.name` only from the whitelist. Adaptive pool, max 3 parallel.
...
- **Sub-agent pool caps at 3.** More parallelism costs more tokens/time than it saves. Small input → single agent.
...
| `scripts/pipeline_parse.py` | Slice xlsx/csv by module + token budget | After scan, with xlsx |
...
| Sub-agent parallelism > 3 | More cost than benefit |
