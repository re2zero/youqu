...
输入：
  --pipeline-a-dir: Pipeline A 输出目录（含 pre_scan_gaps.yaml, pre_scan_ok.yaml, qml_*.yaml）
  --pipeline-b-dir: Pipeline B 输出目录（含 b_report.md 或 b_data.json）
  --output: 报告输出目录
...
def generate_report(
    ...
    data: dict[str, Any] = {
        ...
        "pipeline_a": {"cpp": a_cpp, "qml": a_qml},
        "pipeline_b": b_data,
    }
    ...
    lines.append("## 覆盖率概览")
    lines.append("")
    lines.append("| 分析线 | 语言 | 元素 A（期望） | 元素 B（已有） | 覆盖率 |")
    lines.append("|--------|------|---------------|---------------|--------|")

    if a_cpp:
        lines.append(f"| Pipeline A（静态扫描） | C++ | {a_cpp['total']} | {a_cpp['ok']} | {a_cpp['coverage']}% |")
    if a_qml:
        lines.append(f"| Pipeline A（静态扫描） | QML | {a_qml['total']} | {a_qml['ok']} | {a_qml['coverage']}% |")
    if b_data:
        b_cov = b_data.get("coverage", 0)
        b_total = b_data.get("total", 0)
        b_ok = b_data.get("ok", 0)
        lines.append(f"| Pipeline B（代码图谱） | C++ | {b_total} | {b_ok} | {b_cov}% |")
    lines.append("")
    ...
    if a_cpp:
        lines.append("### Pipeline A（静态扫描 — C++）")
        lines.append("")
    ...
    if a_qml:
        lines.append("### Pipeline A（静态扫描 — QML）")
        lines.append("")
    ...
    if a_cpp and a_cpp["gap_list"]:
        lines.append("### Pipeline A — C++ Gap 列表")
        lines.append("")
    ...
    if a_qml and a_qml["gap_list"]:
        lines.append("### Pipeline A — QML Gap 列表")
        lines.append("")
    ...
    if b_data:
        ...
        if b_gaps:
            lines.append("### Pipeline B — Gap 列表")
            lines.append("")
    ...
    b_cov = b_data.get("coverage", 0) if b_data else 0
    diff = abs(a_cov - b_cov)

    if a_cpp and b_data:
        lines.append(f"- **覆盖率差异**: Pipeline A = {a_cov}%, Pipeline B = {b_cov}%, 差异 = {diff}%")
        lines.append("- **粒度差异**: Pipeline A 是实例级（每个 `m_xxx` 变量），Pipeline B 是类级（每个控件类）")
        lines.append("- **Pipeline A 独有**: 能发现同一类不同实例的命名不一致")
        lines.append("- **Pipeline B 独有**: 能发现头文件中声明的控件类（即使没有实例化）")
        lines.append("")
...
def main():
    ...
    parser.add_argument("--pipeline-a-dir", required=True, help="Pipeline A output directory")
    parser.add_argument("--pipeline-b-data", help="Pipeline B data JSON file")
